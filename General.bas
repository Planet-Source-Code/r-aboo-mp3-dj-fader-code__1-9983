...
Sub UpdateBars(ByVal hwnd As Long, ByVal nIDEvent As Long, ByVal uElapse As Long, ByVal lpTimerFunc As Long)


On Error Resume Next


frmMain.PicScroll1.Max = frmMain.am1.Duration
frmMain.PicScroll1.Value = frmMain.am1.CurrentPosition

frmMain.PicScroll2.Max = frmMain.am2.Duration
frmMain.PicScroll2.Value = frmMain.am2.CurrentPosition


End Sub

Sub UpdateTimes(ByVal hwnd As Long, ByVal nIDEvent As Long, ByVal uElapse As Long, ByVal lpTimerFunc As Long)

On Error GoTo Fail
Dim min, sec, tid As Integer
Dim Second, Length As Long

Length = CInt(frmMain.am1.Duration)
Second = CInt(frmMain.am1.CurrentPosition)

min = Second \ 60
sec = Second - (min * 60)
If sec = -1 Then sec = 0
frmMain.Label3.Caption = min & " min " & sec & " sec"

tid = Length - Second
min = tid \ 60
sec = tid - (min * 60)
If sec = -1 Then sec = 0
frmMain.Label11.Caption = "(remaining: " & min & " min " & sec & " sec)"

Fail:

On Error GoTo Fail2

Length = CInt(frmMain.am2.Duration)
Second = CInt(frmMain.am2.CurrentPosition)

tid = Length - Second
min = tid \ 60
sec = tid - (min * 60)
If sec = -1 Then sec = 0
frmMain.Label12.Caption = "(remaining: " & min & " min " & sec & " sec)"

min = Second \ 60
sec = Second - (min * 60)
If sec = -1 Then sec = 0
frmMain.Label4.Caption = min & " min " & sec & " sec"
